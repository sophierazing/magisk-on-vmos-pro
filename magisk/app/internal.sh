...
run_installer(){
  ...
  for dir in block busybox mirror worker; do
    mkdir -p /sbin/.magisk/"$dir" 2>/dev/null
  done
...
if [ -z "$@" ]; then
  ...
  for module in /data/adb/modules/*; do {
    ...
    #检测状态
    [ -f /data/adb/load-module/backup/remove-"$(basename "$module")".sh -o -f "$module"/skip_mount -o ! -d "$module"/system/ ] && continue
    #重启服务
    if [ -z "$restart" ]; then
      #停止服务
      setprop ctl.stop zygote
