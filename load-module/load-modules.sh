...
for module in /data/adb/modules/*; do
  #检测状态
  [ "$(cat $list | grep "$module")" ] || [ -f "$module/skip_mount" ] || [ -f "$module/disable" ] || [ ! -d "$module/system/" ] && continue
  #重启服务
  [ -z "$restart" ] && stop zygote && stop zygote_secondary && restart=true
  #切换目录
  cd "$module/system"
  ...
  for file in $(find); do
    ...
    if [ -f "$module/system/$target" ]; then
      ...
      #重置权限
      chmod 777 "$module/system/$target"
      #复制文件
      cp -f "$module/system/$target" "/system/$target"
      #设置权限
      chmod 777 "/system/$target"
    elif [ -d "$module/system/$target" ]; then
      #检查目录
      [ -d "/system/$target" ] && continue
      #创建目录
      mkdir -m 777 "/system/$target"
      #修改文件
